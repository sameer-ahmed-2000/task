...
export const NavBar = () => {
    ...
    return (
        <nav className="px-8 bg-[#3074f4] h-12 flex justify-end items-center">
            {walletAddress ? (
                <p>
                    {walletAddress} <button className="bg-[#FFFFFF] rounded-md h-8 w-24 font-bold" onClick={handleDisconnect}>Disconnect</button>
                </p>
            ) : (
                <button className="bg-[#FFFFFF] rounded-md h-8 w-24 font-bold" onClick={handleConnect}>Connect</button>
            )}
        </nav>
    )
}